...
class Database:
    ...
    def __init__(self, db_path: Path):
        ...
        self._conn: Optional[sqlite3.Connection] = None  # Main thread connection
        self._thread_local = threading.local()  # Thread-local storage for connections
        self._connections: Dict[int, sqlite3.Connection] = {}  # Track all connections
        self._connections_lock = threading.Lock()  # Lock for the connections dict
        self._schema_lock = threading.Lock()  # Lock for schema initialization
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # For main thread, create the connection normally
    ...
    def _get_connection(self) -> Optional[sqlite3.Connection]:
        ...
        # If this is the main thread and we have a main connection, use it
        if thread_id == threading.main_thread().ident and self._conn:
            return self._conn
        ...
        try:
            ...
            self.logger.debug(f"Created thread-local database connection for thread {thread_id}")
            return conn
        except sqlite3.Error as e:
            self.logger.error(f"Failed to create thread-local connection: {e}", exc_info=True)
            return None
    ...
    def close(self):
        ...
        if self._conn:
            self.logger.info("Closing main database connection.")
            self._conn.close()
            self._conn = None
        ...
        with self._connections_lock:
            for thread_id, conn in self._connections.items():
                try:
                    ...
                    self.logger.error(f"Error closing connection for thread {thread_id}: {e}")
    ...
    def _set_db_version(self, version: int):
        ...
        if not isinstance(version, int) or version < 0:
            self.logger.error(f"Invalid version number: {version}. Must be a non-negative integer.")
    ...
    def get_slide_origin(self, slide_id: int) -> Optional[Tuple[str, int]]:
        ...
        try:
            ...
            if not full_path.exists():
                self.logger.warning(f"Slide origin file does not exist: {full_path} for slide ID {slide_id}")
                return None
    ...
    def _initialize_schema(self):
        ...
        # Use schema lock to prevent concurrent schema initialization
    ...
    def _add_keyword_if_not_exists_no_lock(self, conn: sqlite3.Connection, keyword_text: str, kind: KeywordKind) -> Optional[int]:
        """
        Internal version of add_keyword_if_not_exists that doesn't acquire locks or commit.
    ...
    def get_keyword_id(self, keyword_text: str, kind: KeywordKind) -> Optional[int]:
        ...
        keyword_text = keyword_text.strip()
        conn = self._get_connection()
        if not keyword_text or not conn: return None
    ...
    def _link_slide_keyword_no_lock(self, conn: sqlite3.Connection, slide_id: int, keyword_id: int) -> bool:
        """
        Internal version of link_slide_keyword that doesn't acquire locks or commit.
        For use within transactions.
        """
    ...
    def _unlink_slide_keyword_no_lock(self, conn: sqlite3.Connection, slide_id: int, keyword_id: int) -> bool:
        """
        Internal version of unlink_slide_keyword that doesn't acquire locks or commit.
        For use within transactions.
        """
        ...
        with QMutexLocker(self._write_mutex):
            try:
                ...
                for text in new_keyword_texts:
                    keyword_id = self._add_keyword_if_not_exists_no_lock(conn, text, kind)
                ...
                for keyword_id in to_remove:
                    success = self._unlink_slide_keyword_no_lock(conn, slide_id, keyword_id)
                    if not success:
                ...
                for keyword_id in to_add:
                    success = self._link_slide_keyword_no_lock(conn, slide_id, keyword_id)
                    if not success:

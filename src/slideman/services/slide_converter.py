...
# --- Local Imports ---
from .database import Database # To interact with DB
...
class SlideConverter(QRunnable):
    ...
    """
    def __init__(self, file_id: int, file_path: Path, db_service: Database, signals: SlideConverterSignals):
        super().__init__()
        ...
        # Store the database service for later use
    ...
    def run(self):
        ...
        # Create a thread-local database connection using the same path as the main DB
        from .database import Database
        thread_local_db = Database(db_path=self.db_service.db_path)
        ...
        try:
            ...
            # --- Connect to DB for this worker ---
            if not thread_local_db.connect():
                logger.error(f"[Converter FID:{self.file_id}] Worker failed to connect to database.")
                self.signals.error.emit(self.file_id, "Worker Database Connection Failed")
                # Make sure COM is uninitialized if it was initialized
                try: pythoncom.CoUninitialize()
                except Exception: pass
                return # Cannot proceed without DB
        # -----------------------------------
        except Exception as e:
            logger.error(f"[Converter FID:{self.file_id}] Failed to initialize COM: {e}", exc_info=True)
            self.signals.error.emit(self.file_id, f"COM Initialization failed: {e}")
